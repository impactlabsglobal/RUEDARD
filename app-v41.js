...
const zones = ["Piantini, Santo Domingo", "Aeropuerto Las Américas (AILA)", "Punta Cana", "Santiago", "La Romana", "Gazcue", "Bella Vista"];
...
function bindNav() {
  ...
  document.querySelector("#menuBtn")?.addEventListener("click", () => document.querySelector("#drawer")?.classList.toggle("open"));
  document.querySelector("#authBtn")?.addEventListener("click", () => user() ? logout() : go("auth"));
  ...
  document.querySelector("#langBtn")?.addEventListener("click", () => toast("Traducción completa se conecta en la próxima fase."));
}
...
function renderChrome() {
  ...
  if (authBtn) authBtn.textContent = u ? "Salir" : "Entrar";
  if (drawerName) drawerName.textContent = u ? u.name : "RuedaRD";
  if (drawerRole) drawerRole.textContent = u ? roleLabel(u.role) : "MVP demo";
}

function initForms() {
  ["quickStart", "rentStart"].forEach((id) => { const el = document.querySelector(`#${id}`); if (el) el.value = today(1); });
  ["quickEnd", "rentEnd"].forEach((id) => { const el = document.querySelector(`#${id}`); if (el) el.value = today(4); });
  ...
  document.querySelector("#quickSearch")?.addEventListener("submit", (e) => {
    e.preventDefault();
    document.querySelector("#rentZone").value = document.querySelector("#quickZone").value;
    document.querySelector("#rentStart").value = document.querySelector("#quickStart").value;
    document.querySelector("#rentEnd").value = document.querySelector("#quickEnd").value;
    go("search");
  });
  ...
  document.querySelectorAll("[data-demo]").forEach((btn) => btn.addEventListener("click", () => setSession(demoUsers[btn.dataset.demo])));
  document.querySelector("#carForm")?.addEventListener("submit", addCar);
  document.querySelector("#checkoutForm")?.addEventListener("submit", confirmBooking);
...
function feePrice(car) { return Number(car.price) + 10; }

function daysBetween() {
  const s = new Date(document.querySelector("#rentStart")?.value || today(1));
  const e = new Date(document.querySelector("#rentEnd")?.value || today(4));
...
function renderCars() {
  ...
  if (!list) return;
  const order = { premium: 0, boost: 1, organic: 2 };
  const cars = db.cars.filter((c) => c.status === "approved").filter((c) => activeFilter === "all" || c.cat === activeFilter).sort((a, b) => order[a.ad] - order[b.ad]);
  list.innerHTML = cars.map((car) => `
    <article class="car-card">
      ...
      <div class="car-body">
        <div class="car-title">
          <div><h3>${car.brand} ${car.model} ${car.year}</h3><p class="muted">📍 ${car.zone} · ${car.ownerName} · ⭐ ${car.rating || "Nuevo"}</p></div>
        ...
        <div class="car-actions"><button class="ghost" type="button" onclick="viewCar('${car.id}')">Ver</button><button class="primary" type="button" onclick="selectCar('${car.id}')">Reservar</button></div>
      </div>
    </article>`).join("") || `<div class="card"><h3>No hay carros en este filtro.</h3><p class="muted">Prueba otra categoría o sube un carro nuevo.</p></div>`;
}

function catLabel(cat) { return { sedan: "Sedan", suv2: "SUV 2 filas", suv3: "SUV 3 filas", luxury: "Lujo" }[cat] || "Carro"; }

window.viewCar = (id) => {
  const car = db.cars.find((c) => c.id === id);
  if (car) toast(`${car.brand} ${car.model}: $${feePrice(car)}/día, seguro se elige después.`);
};

window.selectCar = (id) => {
  if (!user()) { toast("Entra como cliente para reservar."); go("auth"); return; }
...
function renderCheckout() {
  ...
  const subtotal = feePrice(selectedCar) * days;
  document.querySelector("#checkoutSummary").innerHTML = `
    <h3>Resumen de renta</h3>
    <div class="line"><span>Vehículo</span><strong>${selectedCar.brand} ${selectedCar.model}</strong></div>
    <div class="line"><span>Días</span><strong>${days}</strong></div>
    <div class="line"><span>Precio por día</span><strong>$${feePrice(selectedCar)}</strong></div>
...
function protectionCost() {
  const v = document.querySelector("#protection")?.value;
...
function confirmBooking(e) {
  ...
  const total = feePrice(selectedCar) * days + insurance;
  ...
  const booking = {
    ...
    start: document.querySelector("#rentStart").value,
    end: document.querySelector("#rentEnd").value,
    zone: document.querySelector("#rentZone").value,
    airline: document.querySelector("#airline").value,
    flightNo: document.querySelector("#flightNo").value.trim(),
    payment: document.querySelector("#paymentMethod").value,
    protection: document.querySelector("#protection").value,
...
async function addCar(e) {
  ...
  const u = user();
  if (!u) return go("auth");
  if (u.role === "renter") { toast("Cambia a socio o empresa para subir carros."); return; }
  const photos = await readPhotoFiles(document.querySelector("#carPhotos"));
  const car = {
    id: `c-${Date.now()}`,
    ownerId: u.id,
    ownerName: u.name,
    brand: document.querySelector("#carBrand").value,
    model: document.querySelector("#carModel").value.trim(),
    year: Number(document.querySelector("#carYear").value),
    cat: document.querySelector("#carCat").value,
    zone: document.querySelector("#carZone").value,
    seats: Number(document.querySelector("#carSeats").value),
    price: Number(document.querySelector("#carPrice").value),
    ad: document.querySelector("#carAd").value,
    status: "approved",
    docs: [fileName("docInsurance") || "Seguro subido", fileName("docTitle") || "Matrícula/título subido", fileName("docOwner") || "Dueño/empresa subido"],
    photos,
    rating: "Nuevo",
    createdAt: new Date().toISOString()
  };
  ...
  saveDb();
  e.target.reset();
  document.querySelector("#carYear").value = "2024";
  document.querySelector("#carPrice").value = "40";
  document.querySelector("#carSeats").value = "5";
...
function renderDashboard() {
  const u = user();
  if (!u) return go("auth");
  document.querySelector("#dashRole").textContent = roleLabel(u.role).toUpperCase();
  document.querySelector("#dashTitle").textContent = u.role === "renter" ? "Panel del cliente" : u.role === "owner" ? "Panel socio individual" : "Panel empresa rent car";
  ...
  document.querySelector("#stats").innerHTML = [["Reservas", myBookings.length], ["Carros", myCars.length], ["Disponible", `$${wallet.available}`], ["Pendiente", `$${wallet.pending}`]].map(([k, v]) => `<div class="stat"><span class="muted">${k}</span><strong>${v}</strong></div>`).join("");
  document.querySelector("#bookingList").innerHTML = myBookings.map(bookingItem).join("") || `<p class="muted">Todavía no hay reservas.</p>`;
  document.querySelector("#ownerCars").innerHTML = myCars.map(carItem).join("") || `<p class="muted">Todavía no has subido carros.</p>`;
}

function bookingItem(b) {
  return `<div class="item"><div class="item-head"><strong>${b.vehicle}</strong><span class="badge amber">${statusLabel(b.status)}</span></div><p class="muted">${b.start} → ${b.end} · ${b.zone}</p><p><strong>$${b.total}</strong> total · hold $${b.depositHold}</p>${b.status === "hold_pending_owner_confirmation" ? `<button class="primary small" onclick="releaseBooking('${b.id}')">Confirmar entrega / liberar dueño</button>` : ""}</div>`;
}

function carItem(c) {
  return `<div class="item"><div class="item-head"><strong>${c.brand} ${c.model} ${c.year}</strong><span class="badge blue">${c.ad}</span></div><p class="muted">${catLabel(c.cat)} · ${c.zone} · $${c.price} dueño + $10 fee</p></div>`;
}

function statusLabel(s) { return s === "completed" ? "Completada" : "Hold pendiente"; }
...
function renderWallet() {
  ...
  document.querySelector("#walletPanel").innerHTML = `<h3>Wallet demo</h3><div class="wallet-line"><span>Disponible para retirar</span><strong>$${w.available}</strong></div><div class="wallet-line"><span>Pendiente por confirmar</span><strong>$${w.pending}</strong></div><div class="wallet-line"><span>Retirado</span><strong>$${w.withdrawn}</strong></div><button class="primary" onclick="withdrawMoney()">Solicitar retiro demo</button><p class="mini">En producción esto sería ACH/LBTR a BHD, Popular, Banreservas u otra cuenta verificada.</p>`;
}
...
function renderContracts() {
  ...
  document.querySelector("#contractList").innerHTML = contracts.map((c) => `<div class="item"><div class="item-head"><strong>${c.title}</strong><span class="badge green">PDF demo</span></div><pre style="white-space:pre-wrap;color:var(--muted);font-family:inherit">${c.text}</pre><button class="primary small" onclick="downloadContract('${c.id}')">Descargar contrato .txt</button></div>`).join("") || `<p class="muted">No hay contratos todavía. Haz una reserva para generar uno.</p>`;
}
